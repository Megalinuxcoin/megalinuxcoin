...
#if defined(NDEBUG)
# error "Megalinuxcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xcd18a0f0605ff9167aec3a594db9cb0a8a28e906ad4e019132931914712e4ead");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Megalinuxcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Megalinuxcoin Signed Message:\n";
...
{
    // Megalinuxcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
//Number of coins per block awarded
    ...
    nSubsidy >>= (nHeight / 840000); // Megalinuxcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Megalinuxcoin: 1 days
static const int64 nTargetSpacing = 1.5 * 60; // Megalinuxcoin: 1.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Megalinuxcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Megalinuxcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Rome Times 21/Feb/2015 Ruggero Grando, Megalinux";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("043684710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        ...
        if (fTestNet)
        {
            block.nTime    = 1424527923; // Tempo di genesis coin testnet
        ...
        assert(block.hashMerkleRoot == uint256("0x5de525b04bf193a9ab62cba8c8e613d6a8cf9a27a81a741cc28671296b5eb39b"));
...
unsigned char pchMessageStart[4] = { 0xc3, 0xd2, 0xd3, 0xba }; // Megalinuxcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MegalinuxcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MegalinuxcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MegalinuxcoinMiner(CWallet *pwallet)
{
    printf("MegalinuxcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("megalinuxcoin-miner");
    ...
    try { loop {
        ...
        printf("Running MegalinuxcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MegalinuxcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MegalinuxcoinMiner, pwallet));
}
